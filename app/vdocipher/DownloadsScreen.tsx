...
export default function DownloadsScreen(
  props: NativeStackScreenProps<RootStackParamList, 'Downloads'>,
) {
  const [unregister, setUnregister] = useState<Array<() => void>>([]);
  const [downloadStatusArray, setDownloadStatusArray] = useState<
  ...
  useEffect(() => {
    ...
    unregister.push(
      VdoDownload.addEventListener(
        'onQueued',
        (mediaId: string, status: DownloadStatus) => _onQueued(mediaId, status),
      ),
    );
    unregister.push(
      VdoDownload.addEventListener(
        'onChanged',
        (mediaId: string, status: DownloadStatus) =>
    ...
    unregister.push(
      VdoDownload.addEventListener(
        'onFailed',
        (mediaId: string, status: DownloadStatus) => _onFailed(mediaId, status),
      ),
    );
    unregister.push(
  ...
  const _getSelection = (availableTracks: Array<Track>) => {
    var selected: number[] = [];
    var audioIndex = availableTracks.findIndex(
      (track: Track) => track.type === 'audio',
    );
    var videoIndex = availableTracks.findIndex(
      (track: Track) => track.type === 'video',
    );
    if (audioIndex != -1) {
      selected.push(audioIndex);
    }
    if (videoIndex != -1) {
      selected.push(videoIndex);
    } else {
    ...
    VdoDownload.getDownloadOptions({otp, playbackInfo})
      .then(
        ({
          ...
          enqueue: (selections: Array<number>) => void;
        }) => {
  ...
  const _enqueueDownload = (otp: string, playbackInfo: string) => {
    ...
    VdoDownload.getDownloadOptions({
      otp,
      playbackInfo /* ,customPlayerId: "avaI83RCHxfvLwhC"*/,
    })
      .then(
        ({
          downloadOptions,
          ...
          downloadOptions: DownloadOptions;
          enqueue: (downloadSelections: {selections: Array<number>}) => void;
        }) => {
          console.log('Got options', downloadOptions);
          const selections = _getSelection(downloadOptions.availableTracks);
          console.log('selections', selections);
  ...
  const _stopDownload = (mediaId: string) => {
    VdoDownload.stop([mediaId])
      ...
      .catch((error: {exception: string; msg: string}) =>
  ...
  const _resumeDownload = (mediaId: string) => {
    VdoDownload.resume([mediaId])
      .then(() => console.log('resume success'))
      .catch((error: {exception: string; msg: string}) =>
  ...
  const _removeDownload = (mediaId: string) => {
    ...
    VdoDownload.remove([mediaId]).catch(
      (error: {exception: string; msg: string}) =>
        console.warn('Error removing ' + mediaId),
    );
  };

  const _isExpired = (mediaId: string) => {
    VdoDownload.isExpired(mediaId)
      .then((isExpired: boolean) => console.log('expiry: ', isExpired))
      .catch((error: {exception: string; msg: string}) =>
        ...
        <FlatList
          ...
          renderItem={({item, index}: {item: DownloadData; index: number}) => (
            <DownloadListItem
              ...
              onDownload={() => _enqueueDownload(item.otp, item.playbackInfo)}
              onPlay={() =>
