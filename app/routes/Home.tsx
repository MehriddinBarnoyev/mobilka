...
export default function Home() {
    ...
    const fetchUserThenData = useCallback(async (isRefresh: boolean = false) => {
        ...
        try {
            ...
            const userId = u.id
           const response = await api.get<ApiResponse>(`https://assoodiq.devops.uz/services/videoedums/api/user-accesses/media/${userId}`)
                            ...
                            {item.type === "VIDEO" && item.coverImageUrl && (
                                <View style={[styles.thumbnailContainer, { width: thumbnailSize, height: thumbnailSize }]}>
                                    ...
                                    <Ionicons
                                        ...
                                        size={iconSize}
                                        color="#ffffff"
                                        style={[styles.thumbnailIcon, { transform: [{ translateX: -iconSize / 2 }, { translateY: -iconSize / 2 }] }]}
                                    />
                                </View>
                            )}
                            {(item.type !== "VIDEO" || !item.coverImageUrl) && (
    ...
    return (
        <View style={[styles.container, { paddingTop: top }]}>
            ...
            <FlatList
                ...
                refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={["#1ed488"]} tintColor="#1ed488" />}
