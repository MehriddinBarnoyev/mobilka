...
import {DownloadStatus, OfflineEmbedInfo} from 'vdocipher-rn-bridge/type';
...
export default function MyDownloadsScreen({navigation}: Props) {
  ...
  useEffect(() => {
    ...
    const newListeners = [
      VdoDownload.addEventListener('onQueued', ({mediaId, status}: any) =>
        _refreshDownloadList(),
      ),
      VdoDownload.addEventListener('onChanged', ({mediaId, status}: any) =>
        _updateItem(mediaId, status),
      ),
      VdoDownload.addEventListener('onCompleted', ({mediaId, status}: any) =>
        _refreshDownloadList(),
      ),
      VdoDownload.addEventListener('onFailed', ({mediaId, status}: any) =>
        _refreshDownloadList(),
      ),
      VdoDownload.addEventListener('onDeleted', (mediaId: any) =>
        _refreshDownloadList(),
      ),
    ];
  ...
  const _updateItem = (mediaId: string, updatedStatus: DownloadStatus) => {
    const index = downloadStatusArray.findIndex(
      d => d.mediaInfo.mediaId === mediaId,
    );
  ...
  const _handleDelete = (mediaId: string) => {
    Alert.alert('Delete Download', 'Are you sure?', [
      {text: 'Cancel', style: 'cancel'},
      {
        ...
        style: 'destructive',
        onPress: () =>
          VdoDownload.remove([mediaId]).catch((err: any) =>
  ...
  const renderItem = ({item}: {item: DownloadStatus}) => {
    const {mediaInfo, status, downloadPercent, poster, enableAutoResume} = item;
      ...
      <View style={styles.item}>
        ...
        <View style={styles.buttonGroup}>
          <Button
            ...
            onPress={() => _handlePlay(mediaInfo.mediaId, enableAutoResume)}
          />
          <Button
            title="Delete"
            color="red"
            onPress={() => _handleDelete(mediaInfo.mediaId)}
          />
        </View>
      </View>
    );
